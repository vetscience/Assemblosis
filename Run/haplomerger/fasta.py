...
class Fasta(Base):
    ...
    def order(self, reverse = False):
        '''order to length of the ORFs
        '''
        lens = [len(self.seqs[i]) for i in xrange(len(self.headers))]
        self.idxs = self._argsort(lens)
        if reverse == True:
            self.idxs = self.idxs[::-1]
        #fasta.headers = fasta.headers[idxs]
        #fasta.seqs = fasta.seqs[idxs]
        #print ">%s" %fasta.headers[idxs[i]]
        #print "%s" %fasta.seqs[idxs[i]]

        #lens = []
        #for i in xrange(len(self.headers)):
        #    lens.append(len(self.seqs[i]))
        #lens = array(lens)
        #idxs = lens.argsort()[::-1]
        #if reverse == True:
        #    idxs = idxs[::-1]
        #headers = array(self.headers)
        #seqs = array(self.seqs)
        #self.headers = list(headers[idxs])
        #self.seqs = list(seqs[idxs])
